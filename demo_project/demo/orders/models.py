...
class Sprint(models.Model):
    ...
    def totalTaskHours(self):
        ...
        tasks = self.task_set.all()
        for task in tasks:
            tmp+=task.effortHours
        return tmp

    def recordChanges(self,hours):
        date = datetime.date.today()
        tmp = date - self.startDate
        sprintDay = tmp.days
        if sprintDay in self.changes:
            self.changes[sprintDay][0]+=hours
            self.changes[sprintDay][1]=self.totalTaskHours()
        else:
            self.changes[sprintDay]=(hours,self.totalTaskHours())
        self.save()
